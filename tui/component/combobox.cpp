...
GradientDataList ComboBoxPrivate::sGradientList =
...
ComboBoxPrivate::ComboBoxPrivate(ComboBox *q) : m_qptr(q)
{
...
ComboBox::ComboBox(QWidget *parent) : QComboBox(parent), m_dptr(new ComboBoxPrivate(this)) {}
...
QString ComboBox::currentName() const
{
    QListWidgetItem *currItem = m_dptr->gradientItemList->currentItem();
    if (!currItem)
        return QString();
...
QColor ComboBox::currentColor1() const
{
    QListWidgetItem *currItem = m_dptr->gradientItemList->currentItem();
    if (!currItem)
        return QColor();
...
QColor ComboBox::currentColor2() const
{
    QListWidgetItem *currItem = m_dptr->gradientItemList->currentItem();
    if (!currItem)
        return QColor();
...
QPair<QColor, QColor> ComboBox::colorPair(int index)
{
    if (0 <= index && index < ComboBoxPrivate::sGradientList.size())
        return qMakePair(ComboBoxPrivate::sGradientList.at(index).color1, ComboBoxPrivate::sGradientList.at(index).color2);
    else
