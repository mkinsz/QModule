...
GradientWidgetItem::GradientWidgetItem(const GradientData &gradient,
                                       QWidget *parent)
    : QWidget(parent), m_gradientData(gradient)
{
    ...
    nameLabel->setSizePolicy(QSizePolicy(QSizePolicy::Minimum, QSizePolicy::Minimum));
...
GradientDataList GComboBoxPrivate::sGradientList =
...
GComboBoxPrivate::GComboBoxPrivate(GComboBox *q) : m_qptr(q)
{
    gradientItemList = new QListWidget(m_qptr);
    gradientItemList->setAlternatingRowColors(true);
    m_qptr->connect(m_qptr, SIGNAL(activated(int)), m_qptr, SLOT(onActivated(int)));
    ...
    m_qptr->setModel(gradientItemList->model());
    m_qptr->setView(gradientItemList);

    gradientItemList->setCurrentRow(0);
}

GComboBox::GComboBox(QWidget *parent) : QComboBox(parent), m_dptr(new GComboBoxPrivate(this)) {}
...
void GComboBox::setCurrentIndex(int index)
{
    m_dptr->gradientItemList->setCurrentRow(index);
    QListWidgetItem *currItem = m_dptr->gradientItemList->currentItem();
    GradientWidgetItem *gwi = m_dptr->listItem2GradientItemMap.value(currItem);
    if (gwi)
        setCurrentText(gwi->name());
}

int GComboBox::currentIndex() const
{
    return m_dptr->gradientItemList->currentRow();
}

QString GComboBox::currentName() const
{
    QListWidgetItem *currItem = m_dptr->gradientItemList->currentItem();
    if (!currItem)
        return QString();

    return m_dptr->listItem2GradientItemMap.value(currItem)->name();
}

QColor GComboBox::currentColor1() const
{
    QListWidgetItem *currItem = m_dptr->gradientItemList->currentItem();
    if (!currItem)
        return QColor();

    return m_dptr->listItem2GradientItemMap.value(currItem)->color1();
}

QColor GComboBox::currentColor2() const
{
    QListWidgetItem *currItem = m_dptr->gradientItemList->currentItem();
    if (!currItem)
        return QColor();

    return m_dptr->listItem2GradientItemMap.value(currItem)->color2();
}

QPair<QColor, QColor> GComboBox::colorPair(int index)
{
    if (0 <= index && index < GComboBoxPrivate::sGradientList.size())
        return qMakePair(GComboBoxPrivate::sGradientList.at(index).color1, GComboBoxPrivate::sGradientList.at(index).color2);
    else
...
void GComboBox::showPopup()
{
    ...
    int xPos = qRect.width() - m_dptr->gradientItemList->width();
    int yPos = qRect.height() - m_dptr->gradientItemList->height();
    m_dptr->gradientItemList->move(xPos, yPos);
    m_dptr->gradientItemList->doItemsLayout();
    m_dptr->gradientItemList->setVisible(true);
}

void GComboBox::hidePopup()
{
    QComboBox::hidePopup();
    m_dptr->gradientItemList->setVisible(false);
    int currRow = m_dptr->gradientItemList->currentRow();
    emit currentIndexChanged(currRow);
}

void GComboBox::paintEvent(QPaintEvent *e)
{
    QListWidgetItem *item = m_dptr->gradientItemList->currentItem();
    if (!item)
    {
    ...
    GradientWidgetItem *gwi = m_dptr->listItem2GradientItemMap.value(item);

    QRect gradientRect(QPoint(128, 8), QPoint(width() - 20, height() - 8));
...
void GComboBox::resizeEvent(QResizeEvent *e)
{
...
QSize GComboBox::minimumSizeHint() const
{
    return QSize(240, 20);
}

void GComboBox::onActivated(int index)
{
    m_dptr->gradientItemList->setCurrentRow(index);
    update();
}

#include "moc_gcombobox.cpp"
