...
Keyboard::~Keyboard()
{
    for (int i = 0; i < 14; i++)
    {
        ...
        if (i < 13)
            delete btnLetterAl[i];
        if (i < 12)
            delete btnLetterZm[i];

        if (i < 7)
            delete btnBottom[i];
        if (i < 4)
            delete btnDirection[i];
    }
...
void Keyboard::InitButtons()
{
    ...
    connect(btnTop[13], SIGNAL(clicked()), qApp, SLOT(quit()));
    ...
    btnDirection[0]->setIcon(QIcon(":/images/icons/arrow_up.png"));
    btnDirection[1]->setIcon(QIcon(":/images/icons/arrow_left.png"));
    btnDirection[2]->setIcon(QIcon(":/images/icons/arrow_down.png"));
    btnDirection[3]->setIcon(QIcon(":/images/icons/arrow_right.png"));
