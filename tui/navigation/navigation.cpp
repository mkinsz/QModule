                        ...
                        const QModelIndex &index) const
{
    ...
    if (node->children.size() != 0)
    {
        ...
        if (nav->getIcoStyle() == Navigation::IcoStyle_Cross)
        {
        ...
        else if (nav->getIcoStyle() == Navigation::IcoStyle_Triangle)
        {
            p.setPen(Qt::NoPen);
...
void NavModel::updateData(QStringList listItem)
{
...
QVariant NavModel::data(const QModelIndex &index, int role) const
{
    ...
    if ((uint)index.row() >= listNode.size() || index.row() < 0)
        return QVariant();
    ...
    if (role == Qt::DisplayRole)
        ...
        return (uint)listNode[index.row()].treeNode;
...
Navigation::Navigation(QWidget *parent) : QListView(parent)
{
    infoVisible = true;
    lineVisible = true;
    icoColorBg = false;

    style = Navigation::IcoStyle_Cross;
    ...
    m_pModel = new NavModel(this);
    m_pDelegate = new NavDelegate(this);
    connect(this, SIGNAL(doubleClicked(QModelIndex)), m_pModel, SLOT(collapse(QModelIndex)));
}
...
bool Navigation::getInfoVisible() const
{
    return infoVisible;
}

bool Navigation::getLineVisible() const
{
    return lineVisible;
}

bool Navigation::getIcoColorBg() const
{
    return icoColorBg;
}

Navigation::IcoStyle Navigation::getIcoStyle() const
{
    return style;
}

QColor Navigation::getColorLine() const
{
    return colorLine;
}

QColor Navigation::getColorBgNormal() const
{
    return colorBgNormal;
}

QColor Navigation::getColorBgSelected() const
{
    return colorBgSelected;
}

QColor Navigation::getColorBgHover() const
{
    return colorBgHover;
}

QColor Navigation::getColorTextNormal() const
{
    return colorTextNormal;
}

QColor Navigation::getColorTextSelected() const
{
    return colorTextSelected;
}

QColor Navigation::getColorTextHover() const
{
    return colorTextHover;
}

void Navigation::readData(QString xmlPath)
{
    m_pModel->readData(xmlPath);
    this->setModel(m_pModel);
    this->setItemDelegate(m_pDelegate);
}

void Navigation::setData(QStringList listItem)
{
    m_pModel->updateData(listItem);
    this->setModel(m_pModel);
    this->setItemDelegate(m_pDelegate);
}

void Navigation::setInfoVisible(bool infoVisible)
{
    this->infoVisible = infoVisible;
}

void Navigation::setLineVisible(bool lineVisible)
{
    this->lineVisible = lineVisible;
}

void Navigation::setIcoColorBg(bool icoColorBg)
{
    this->icoColorBg = icoColorBg;
}

void Navigation::setIcoStyle(Navigation::IcoStyle style)
{
    this->style = style;
}

void Navigation::setColorLine(QColor colorLine)
{
    this->colorLine = colorLine;
}

void Navigation::setColorBg(QColor colorBgNormal, QColor colorBgSelected, QColor colorBgHover)
{
...
void Navigation::setColorText(QColor colorTextNormal, QColor colorTextSelected, QColor colorTextHover)
{
